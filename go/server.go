...
func StoreAccountData(c *gin.Context) {
	...
	getAccessToken(c, publicToken)
	err := CreateJsonAccountData(username)
	if err != nil {
		renderError(c, err)
		c.JSON(http.StatusInternalServerError, gin.H{"message": "Signup failed"})
		return
...
func GetAllTransactions(c *gin.Context) {
	...
	cursor, transactions := transactions(c)
...
func transactions(c *gin.Context) (string, []plaid.Transaction) {
	ctx := context.Background()
